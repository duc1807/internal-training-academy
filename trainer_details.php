...
$query =
    "SELECT trainer.name, su.username, trainer.email, trainer.phone_number,
        trainer.is_contractor, d.name, d.location, trainer.self_description
     FROM trainer
     INNER JOIN system_user su ON trainer.user_id = su.id
     LEFT JOIN department d ON trainer.department_id = d.id
     WHERE trainer.id = '$thisId'";
...
$thisEmail = $row[2];
$thisPhoneNum = $row[3];
$thisWorkStatus = ($row[4] == 0) ? "Full-time" : "Contractor";
...
<head>
    ...
    <title>Details of Trainer ID <?php echo $thisId; ?></title>
                ...
                <div class="card mt-5 ml-5 pt-3 pl-3">
                    <div class="table-responsive card-body">
                        <table class="table">
                            ...
                            <tr>
                                <td><h5 class=" font-weight-bold text-dark">Trainer ID</h5></td>
                                <td>
                                    <?php echo $thisId; ?>
                                </td>
                            </tr>
                            <tr>
                                <td><h5 class=" font-weight-bold text-dark">Trainer Name</h5></td>
                                <td>
                                    <?php echo $thisName; ?>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td><h5 class=" font-weight-bold text-dark">Contractor/Full-time?</h5></td>
                                <td>
                                    <?php echo $thisWorkStatus; ?>
                                </td>
                            </tr>
                            <tr>
